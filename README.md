...
Optional secrets are `LANGSMITH_DATASET` (default `rag-regression`), `LANGSMITH_MIN_SCORE` (default `1.0`), `OPENCODE_MODEL` (default `glm-5.3-flash`), `AZURE_LOCATION` (default `eastus`), and `AZURE_APP_SERVICE_PLAN` (default `rag-linux-plan`). `LANGSMITH_WORKSPACE_ID` should identify the workspace that owns the dataset. The deployment workflow creates the resource group, Linux App Service plan, and both Web Apps when they do not already exist, then configures the custom containers and runtime settings.
